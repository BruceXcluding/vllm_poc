...
def main(args: argparse.Namespace):
    ...
    for output_len in output_lens:
        ...
        if output_len == 1:
            ...
            results["prefill_tps"] = (args.input_len * args.batch_size) / results["TTFT"]
        else:
            results["avg_latency"] = (np.mean(latencies))
            results["avg_decode"] = (np.mean(latencies) - results["TTFT"]) / ( args.output_len - 1)
            # results["avg_decode_per_token"] = (results["avg_decode"] / args.batch_size)
            results["decode_tps"] = (args.batch_size / results["avg_decode"])
            results["total_tps"] = ((args.input_len + args.output_len) * args.batch_size) / results["avg_latency"]
    ...
    print("====Metrics Throughput (tok/s)====")
    print('Metrics Prefill TPS: {:.2f}'.format(results["prefill_tps"]))
    print('Metrics Prefill TPS per TP: {:.2f}'.format(results["prefill_tps"] / args.tensor_parallel_size))
    if "decode_tps" in results :
        print('Metrics Decode TPS: {:.2f}'.format(results["decode_tps"]))
        print('Metrics Decode TPS per TP: {:.2f}'.format(results["decode_tps"] / args.tensor_parallel_size))
        print('Metrics Total TPS: {:.2f}'.format(results["total_tps"]))
        print('Metrics Total TPS per TP: {:.2f}'.format(results["total_tps"] / args.tensor_parallel_size))
    print('\n')
